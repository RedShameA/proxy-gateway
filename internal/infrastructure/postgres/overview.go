...
func (r OverviewRepository) LoadResourceCounts(ctx context.Context) (appoverview.ResourceCounts, error) {
	...
	if err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM nodes n JOIN node_observations o ON n.id = o.node_id WHERE o.usable = true AND n.enabled = true`).Scan(&counts.UsableNodes); err != nil {
		return appoverview.ResourceCounts{}, err
	}
...
var _ appoverview.Repository = OverviewRepository{}
