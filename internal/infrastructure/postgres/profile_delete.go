...
func (r ProfileDeleteRepositoryTx) DeleteCredentials(profileID string) error {
	_, err := r.tx.Exec(`DELETE FROM proxy_credentials WHERE profile_id = $1`, profileID)
	return err
}

func (r ProfileDeleteRepositoryTx) DeleteMaintenanceRuns(profileID string) error {
	_, err := r.tx.Exec(`DELETE FROM maintenance_runs WHERE target_id = $1`, profileID)
	return err
}

func (r ProfileDeleteRepositoryTx) RetainedNodeIDs(profileID string) ([]string, error) {
	rows, err := r.tx.Query(`SELECT node_id FROM retained_profile_nodes WHERE profile_id = $1`, profileID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r ProfileDeleteRepositoryTx) DeleteRetainedNodes(profileID string) error {
	_, err := r.tx.Exec(`DELETE FROM retained_profile_nodes WHERE profile_id = $1`, profileID)
	return err
}

func (r ProfileDeleteRepositoryTx) DeleteProfile(profileID string) (int64, error) {
	result, err := r.tx.Exec(`DELETE FROM access_profiles WHERE id = $1`, profileID)
	if err != nil {
		return 0, err
	}
