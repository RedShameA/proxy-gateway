...
func releaseRetainedProfileNodesExcept(ctx context.Context, tx *sql.Tx, profileID string, keepNodeIDs []string) ([]string, error) {
	...
	for _, nodeID := range releaseNodeIDs {
		if _, err := tx.ExecContext(ctx, `DELETE FROM retained_profile_nodes WHERE profile_id = $1 AND node_id = $2`, profileID, nodeID); err != nil {
			return nil, err
		}
	}
...
func retainedNodeIDsForProfile(ctx context.Context, tx *sql.Tx, profileID string) ([]string, error) {
	rows, err := tx.QueryContext(ctx, `SELECT node_id FROM retained_profile_nodes WHERE profile_id = $1`, profileID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func cleanupNodesWithoutReferences(ctx context.Context, tx *sql.Tx, nodeIDs []string) ([]string, error) {
	...
	for _, nodeID := range nodeIDs {
		...
		if err := tx.QueryRowContext(ctx, `SELECT count(*) FROM node_sources WHERE node_id = $1`, nodeID).Scan(&remainingSources); err != nil {
			return nil, err
		}
		...
		if err := tx.QueryRowContext(ctx, `SELECT count(*) FROM retained_profile_nodes WHERE node_id = $1`, nodeID).Scan(&retainedProfiles); err != nil {
			return nil, err
		}
		...
		if _, err := tx.ExecContext(ctx, `DELETE FROM node_observations WHERE node_id = $1`, nodeID); err != nil {
			return nil, err
		}
		if _, err := tx.ExecContext(ctx, `DELETE FROM nodes WHERE id = $1`, nodeID); err != nil {
			return nil, err
		}
...
func nodeRuntimeFingerprint(ctx context.Context, tx *sql.Tx, nodeID string) (string, error) {
	...
	err := tx.QueryRowContext(ctx, `SELECT fingerprint, outbound_json FROM nodes WHERE id = $1`, nodeID).Scan(&fingerprint, &outboundJSON)
...
func resetDynamicProfilesForRemovedCurrentNode(ctx context.Context, tx *sql.Tx, nodeID string) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&profile.id, &profile.autoEval); err != nil {
			_ = rows.Close()
			return err
		}
	...
	for _, profile := range profiles {
		if profile.autoEval {
			if _, err := tx.ExecContext(
				...
				`UPDATE access_profiles
				    SET current_node_id = '',
				        current_exit_node_id = CASE WHEN current_exit_node_id = $1 THEN '' ELSE current_exit_node_id END,
				        state = 'waiting_observation',
				        last_error = '',
				        switch_reason = 'current_node_removed',
				        last_evaluation_started_at = $2
				  WHERE id = $3`,
				nodeID,
				nowMillis,
				profile.id,
