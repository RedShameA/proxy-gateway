...
func (r SystemSettingsRepository) SaveEvaluation(ctx context.Context, settings appsettings.EvaluationSettings) error {
	_, err := r.db.ExecContext(
		...
		`UPDATE evaluation_settings
		 SET global_concurrency = $1,
		     default_min_evaluation_interval_seconds = $2,
		     single_candidate_limit = $3,
		     chain_candidate_limit = $4,
...
func (r SystemSettingsRepository) SaveMaintenance(ctx context.Context, settings appsettings.MaintenanceSettings) error {
	_, err := r.db.ExecContext(
		...
		`UPDATE maintenance_settings
		    SET subscription_refresh_seconds = $1,
		        node_observation_seconds = $2,
		        profile_evaluation_seconds = $3,
		        chain_evaluation_seconds = $4,
		        geoip_update_time = $5,
		        egress_ip_probe_url = $6,
		        subscription_concurrency = $7,
		        node_observation_concurrency = $8,
		        profile_evaluation_concurrency = $9,
		        geoip_concurrency = $10
		  WHERE id = 1`,
		settings.SubscriptionRefreshSeconds,
		settings.NodeObservationSeconds,
		settings.ProfileEvaluationSeconds,
...
var _ appsettings.SystemRepository = SystemSettingsRepository{}
