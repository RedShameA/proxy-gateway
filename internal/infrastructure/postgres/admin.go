...
func (r AdminRepository) CreateCredential(ctx context.Context, record appadmin.CredentialRecord) error {
	_, err := r.db.ExecContext(
		ctx,
		`INSERT INTO admin_credentials (id, password_hash, created_at) VALUES (1, $1, $2)`,
		record.PasswordHash,
		record.CreatedAt,
	)
	return err
...
func (r AdminRepository) CreateSession(ctx context.Context, record appadmin.SessionRecord) error {
	_, err := r.db.ExecContext(
		ctx,
		`INSERT INTO admin_sessions (token_hash, created_at) VALUES ($1, $2)`,
		record.TokenHash,
		record.CreatedAt,
	)
	return err
...
func (r AdminRepository) LoadSessionCreatedAt(ctx context.Context, tokenHash string) (int64, bool, error) {
	var createdAt int64
	err := r.db.QueryRowContext(ctx, `SELECT created_at FROM admin_sessions WHERE token_hash = $1`, tokenHash).Scan(&createdAt)
	if err == sql.ErrNoRows {
		return 0, false, nil
	}
	if err != nil {
...
func (r AdminRepository) DeleteExpiredSessions(ctx context.Context, cutoffMillis int64) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM admin_sessions WHERE created_at <= $1`, cutoffMillis)
	return err
}
...
func (r AdminRepository) UpdatePasswordAndDeleteSessions(ctx context.Context, passwordHash string) error {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE admin_credentials SET password_hash = $1`, passwordHash); err != nil {
		return err
	}
...
var _ appadmin.Repository = AdminRepository{}
