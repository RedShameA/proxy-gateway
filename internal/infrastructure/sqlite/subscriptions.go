...
func (r SubscriptionRepository) UpdateAutoRefresh(ctx context.Context, id string, enabled *bool, intervalSeconds *int, updatedAt int64) (bool, error) {
	var exists int
	if err := r.db.QueryRowContext(ctx, `SELECT 1 FROM subscriptions WHERE id = ?`, id).Scan(&exists); err == sql.ErrNoRows {
		return false, nil
	} else if err != nil {
		return false, err
	}
	if enabled != nil {
		...
		if _, err := r.db.ExecContext(ctx, `UPDATE subscriptions SET auto_refresh_enabled = ?, updated_at = ? WHERE id = ?`, value, updatedAt, id); err != nil {
			return false, err
		}
	}
	if intervalSeconds != nil {
		if _, err := r.db.ExecContext(ctx, `UPDATE subscriptions SET auto_refresh_interval_seconds = ?, updated_at = ? WHERE id = ?`, *intervalSeconds, updatedAt, id); err != nil {
			return false, err
		}
	}
	return true, nil
...
func (r SubscriptionRepository) StoreRefreshError(ctx context.Context, id, errorText string, updatedAt int64) error {
	_, err := r.db.ExecContext(ctx, `UPDATE subscriptions SET last_error = ?, updated_at = ? WHERE id = ?`, errorText, updatedAt, id)
	return err
}
...
func scanSubscriptionRecord(row subscriptionRecordScanner) (appsubscriptions.Record, error) {
	...
	record.AutoRefreshEnabled = autoRefreshEnabled == 1
	return record, err
}
