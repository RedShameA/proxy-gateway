...
func (r ProfileCredentialRepository) LoadProfileIdentifier(ctx context.Context, profileID string) (string, bool, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT profile_identifier FROM access_profiles WHERE id = ?`, profileID).Scan(&identifier)
	if err == sql.ErrNoRows {
		return "", false, nil
	}
	if err != nil {
...
func (r ProfileCredentialRepository) PasswordExists(ctx context.Context, profileID, password string) (bool, error) {
	var exists int
	err := r.db.QueryRowContext(ctx, `SELECT 1 FROM proxy_credentials WHERE profile_id = ? AND password = ?`, profileID, password).Scan(&exists)
	if err == sql.ErrNoRows {
		return false, nil
	}
	if err != nil {
...
func (r ProfileCredentialRepository) ListCredentials(ctx context.Context, profileID string) ([]appprofiles.CredentialRecord, error) {
	...
	for rows.Next() {
		...
		var enabled int
		if err := rows.Scan(&record.ID, &record.ProfileID, &record.Remark, &record.Password, &enabled, &record.CreatedAt, &record.LastUsedAt); err != nil {
			return nil, err
		}
...
func (r ProfileCredentialRepository) SetCredentialEnabled(ctx context.Context, profileID, credentialID string, enabled bool) (bool, error) {
	...
	result, err := r.db.ExecContext(ctx, `UPDATE proxy_credentials SET enabled = ? WHERE id = ? AND profile_id = ?`, value, credentialID, profileID)
	if err != nil {
		return false, err
	}
...
func (r ProfileCredentialRepository) DeleteCredential(ctx context.Context, profileID, credentialID string) (bool, error) {
	result, err := r.db.ExecContext(ctx, `DELETE FROM proxy_credentials WHERE id = ? AND profile_id = ?`, credentialID, profileID)
	if err != nil {
		return false, err
	}
	affected, err := result.RowsAffected()
...
func (r ProfileCredentialRepository) CountCredentials(ctx context.Context, profileID string) (appprofiles.CredentialCounts, error) {
	...
	if err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM proxy_credentials WHERE profile_id = ?`, profileID).Scan(&counts.Total); err != nil {
		return appprofiles.CredentialCounts{}, err
	}
	if err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM proxy_credentials WHERE profile_id = ? AND enabled = 1`, profileID).Scan(&counts.Enabled); err != nil {
		return appprofiles.CredentialCounts{}, err
	}
	return counts, nil
}
