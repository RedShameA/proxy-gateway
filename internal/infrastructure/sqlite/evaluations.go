...
func (r EvaluationRepository) ListTargets(ctx context.Context) ([]appevaluations.TargetRecord, error) {
	rows, err := r.db.QueryContext(ctx, targetSelectSQL+`
...
func (r EvaluationRepository) LoadTarget(ctx context.Context, profileID string) (appevaluations.TargetRecord, bool, error) {
	target, err := scanEvaluationTarget(r.db.QueryRowContext(ctx, targetSelectSQL+` WHERE id = ?`, profileID))
...
func (r EvaluationRepository) CurrentConfigVersion(ctx context.Context, profileID string) (int64, error) {
	var current int64
	err := r.db.QueryRowContext(ctx, `SELECT config_version FROM access_profiles WHERE id = ?`, profileID).Scan(&current)
	if err == sql.ErrNoRows {
		return 0, nil
	}
	return current, err
...
func (r EvaluationRepository) LastError(ctx context.Context, profileID string) (string, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT last_error FROM access_profiles WHERE id = ?`, profileID).Scan(&lastError)
	if err == sql.ErrNoRows {
		return "", nil
	}
...
func (r EvaluationRepository) CurrentPathLatency(ctx context.Context, profileID string) (int64, error) {
	var latency int64
	err := r.db.QueryRowContext(ctx, `SELECT current_path_latency_ms FROM access_profiles WHERE id = ?`, profileID).Scan(&latency)
	if err == sql.ErrNoRows {
		return 0, nil
	}
	return latency, err
...
func (r EvaluationRepository) CurrentNodeID(ctx context.Context, profileID string) (string, error) {
	var nodeID string
	err := r.db.QueryRowContext(ctx, `SELECT current_node_id FROM access_profiles WHERE id = ?`, profileID).Scan(&nodeID)
	if err == sql.ErrNoRows {
		return "", nil
	}
	return nodeID, err
...
func (r EvaluationRepository) CurrentChainPath(ctx context.Context, profileID string) (appevaluations.ChainPath, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT current_node_id, current_exit_node_id FROM access_profiles WHERE id = ?`, profileID).Scan(&path.FrontNodeID, &path.ExitNodeID)
	if err == sql.ErrNoRows {
...
func (r EvaluationRepository) CandidateEgressCountry(ctx context.Context, nodeID string) (string, bool, error) {
	var country string
	err := r.db.QueryRowContext(ctx, `SELECT egress_country FROM node_observations WHERE node_id = ? AND usable = 1`, nodeID).Scan(&country)
	if err == sql.ErrNoRows {
		return "", false, nil
	}
	if err != nil {
...
func (r EvaluationRepository) ListCandidateSourceRefs(ctx context.Context, nodeID string) ([]appevaluations.SourceRef, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT source_type, source_id FROM node_sources WHERE node_id = ?`, nodeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r EvaluationRepository) UpdateProfileState(ctx context.Context, profileID string, configVersion int64, update appevaluations.StateUpdate) (bool, error) {
	return updateProfileState(ctx, r.db, profileID, configVersion, update)
}
...
func (r EvaluationRepository) UpdateProfileStateAndReleaseRetained(ctx context.Context, profileID string, configVersion int64, keepNodeIDs []string, releaseRetained bool, update appevaluations.StateUpdate) (appevaluations.StateReleaseResult, error) {
	...
	updated, err := updateProfileState(ctx, tx, profileID, configVersion, update)
	if err != nil || !updated {
...
const targetSelectSQL = `SELECT id, type, fixed_node_id, exit_node_ids_json, chain_evaluation_mode, test_url,
...
func scanEvaluationTarget(row evaluationTargetScanner) (appevaluations.TargetRecord, error) {
	...
	var manualOnly, nodeStickyEnabled int
	err := row.Scan(
		&target.ID,
		&target.Type,
		&target.FixedNodeID,
	...
	target.ManualOnly = manualOnly == 1
	target.NodeStickyEnabled = nodeStickyEnabled == 1
	return target, nil
}

type profileStateExecutor interface {
	ExecContext(ctx context.Context, query string, args ...any) (sql.Result, error)
}

func updateProfileState(ctx context.Context, exec profileStateExecutor, profileID string, configVersion int64, update appevaluations.StateUpdate) (bool, error) {
	assignments, args, err := profileStateAssignments(update)
	if err != nil {
		return false, err
	}
	query := `UPDATE access_profiles SET ` + strings.Join(assignments, ", ") + ` WHERE id = ? AND (config_version = ? OR ? = 0)`
...
func profileStateAssignments(update appevaluations.StateUpdate) ([]string, []any, error) {
	assignments := []string{}
	args := []any{}
