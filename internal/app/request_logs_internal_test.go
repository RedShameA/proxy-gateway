...
func TestAsyncRequestLogWriterFlushesQueuedEvents(t *testing.T) {
	...
	logID := g.requestLogService.Start(path, "example.test:443", time.Now())
	if logID == "" {
		t.Fatal("request log service returned empty log id")
	}
	g.requestLogService.Finish(logID, true, "", "", 200, 12, 34, 56)
