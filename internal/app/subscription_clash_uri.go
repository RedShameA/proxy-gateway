...
func parseClashProxyMaps(proxies []map[string]any) ([]parsedSubscriptionNode, skippedEntrySummarySet) {
	nodes := make([]parsedSubscriptionNode, 0, len(proxies))
...
func parseURILines(text string) ([]parsedSubscriptionNode, skippedEntrySummarySet) {
...
func missingParsedNodeRequiredField(node parsedSubscriptionNode) bool {
...
func parsedNodeSingBoxOutboundJSON(node parsedSubscriptionNode) string {
...
func parseVMessURI(line string) (parsedSubscriptionNode, error) {
	...
	if !ok {
		return parsedSubscriptionNode{}, errors.New("invalid vmess payload")
	}
	var uri struct {
		Name     string `json:"ps"`
	...
	if err != nil || port <= 0 {
		return parsedSubscriptionNode{}, errors.New("invalid vmess port")
	}
