...
func newSingBoxNodeProtocolEngine() (*singBoxNodeProtocolEngine, error) {
...
func (e *singBoxNodeProtocolEngine) DialNode(node nodeRecord, target string, timeouts dialTimeouts) (net.Conn, error) {
	if !node.Enabled {
...
func (e *singBoxNodeProtocolEngine) DialChain(frontNode, exitNode nodeRecord, target string, timeouts dialTimeouts) (net.Conn, error) {
	if !frontNode.Enabled || !exitNode.Enabled {
		return nil, errors.New("node disabled")
	}
	if normalizeNodeType(exitNode.Type) == "direct" {
...
func runtimeOutboundJSONForNode(node nodeRecord) (string, string, error) {
	outboundJSON := strings.TrimSpace(node.OutboundJSON)
	if outboundJSON == "" {
		var err error
		outboundJSON, err = normalizedNodeOutboundJSON(parsedSubscriptionNode{
			Type:       node.Type,
			Server:     node.Server,
	...
	return outboundJSON, outboundFingerprint(outboundJSON), nil
}
...
func dialContextForTimeouts(timeouts dialTimeouts) (context.Context, context.CancelFunc) {
